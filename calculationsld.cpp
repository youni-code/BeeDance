...
double CalculationSLD::delta_sigma_a() const
{
    double d_sigma(0);
    for(auto &it : elements_)
        d_sigma += (it.index() / c_summ()) * pow(it.element().sigma_a_err(), 2);
    return c_summ() * sqrt(d_sigma);
}
...
double CalculationSLD::delta_sigma_i() const
{
    double sigma(0);
    for(auto &it : elements_)
        sigma += (it.index() / c_summ()) * pow(it.element().sigma_i_err(), 2);
    return c_summ() * sqrt(sigma);
}
...
double CalculationSLD::delta_sigma_t() const
{
    ...
    for(auto &it : elements_)
    {
        ...
        sub_result +=  (it.index() / c_summ()) * pow(mod(val) * mod(delta_val), 2);
    }
    return 4 * pi * 2 * c_summ() * sqrt(sub_result) * 1e-2;
...
double CalculationSLD::delta_b_c() const
{
    double result(0);
    for(auto &it : elements_)
        result += (it.index() / c_summ()) * pow(it.element().bc_comp_err().real(), 2);
    return c_summ() * sqrt(result);
}
...
double CalculationSLD::sld_re()        const { return  c_sld * density_ / a_mass() * b_c(); }
double CalculationSLD::d_sld_re()  const { return  c_sld * density_ / a_mass() * delta_b_c(); }
...
double CalculationSLD::d_v_re()    const { return c_sldV * d_sld_re(); }


double CalculationSLD::v_im()       const { return c_sldV * sld_im(); }
double CalculationSLD::d_v_im() const { return c_sldV * d_sld_im(); }

double CalculationSLD::lambda_c() const
{
    if(v_re() > 0.0) return c_lambda / std::sqrt(v_re());
    return 0.0;
}

double CalculationSLD::d_lambda_c() const
{
    if(v_re() > 0.0)
        return (lambda_c() / 2) * (d_v_re() / v_re());
    return 0.0;
}
...
double CalculationSLD::d_tetta_c() const
{
    if(lambda_c() > 0.0)
        return tetta_c() * (d_lambda_c() / lambda_c());
    return 0.0;
}
...
double CalculationSLD::d_q_c() const
{
    if(lambda_c() > 0.0)
        return q_c() * (d_lambda_c() / lambda_c());
    return 0.0;
}

double CalculationSLD::mu_a() const       { return c_sigma_mu * density_ * sigma_a() / (a_mass()) * (lambda_ / lambda_0); }
double CalculationSLD::d_mu_a() const { return c_sigma_mu * density_ * delta_sigma_a() / (a_mass()) * (lambda_ / lambda_0); }

double CalculationSLD::mu_i() const       { return c_sigma_mu * density_ * (sigma_i() + sigma_i_elem()) / a_mass(); }
double CalculationSLD::d_mu_i() const { return c_sigma_mu * density_ * sqrt(pow(delta_sigma_i(), 2) + pow(delta_sigma_i_elem(), 2)) / a_mass(); }

double CalculationSLD::mu() const         { return mu_a() + mu_i(); }
double CalculationSLD::d_mu() const   { return d_mu_a() + d_mu_i(); }
